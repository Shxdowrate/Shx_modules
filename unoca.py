                                ...
                                if chat != None:
                                    ...
                                    if opst == False:
                                        ...
                                        if limit != None:
                                            ...
                                            await utils.asnwer(message, f'<emoji document_id=5877477244938489129>🚫</emoji> <b>Error |</b> <code>{prefix}uoc</code>\nНо ты же даже не указал свой лимтит... Сколько мне открывать {args[0]} за раз? Посмотри {prefix}usettings')
                                            return
                                    else:
                                        await utils.answer(message, f'<emoji document_id=5877477244938489129>🚫</emoji> <b>Error |</b> <code>{prefix}uoc</code>\nМы пытались открывать кейсы, но... Они уже открываются. Подождите, пожалуйста.')
                                        return
                                else:
                                    await utils.answer(message, f'<emoji document_id=5877477244938489129>🚫</emoji> <b>Error |</b> <code>{prefix}uoc</code>\nА где мне открывать твои {val} {args[0]}? Установи сперва чат, где мне открывать кейсы.')
                                    return
                            else:
                                await utils.asnwer(message, f'<emoji document_id=5877477244938489129>🚫</emoji> <b>Error |</b> <code>{prefix}uoc</code>\nА раз в сколько секундд мне открывать {args[0]}? Я могу конечно и ЗАСПАМИТЬ ЧАТ, но думаю, никто не скажет спасибо, даже MineEVO... Посмотри {prefix}usettings')
                                return
                        else:
