...
class EvDC(loader.Module):
        ...
        if self.get('group') != 0:
            if self.get('status') == True:
                ...
                if self.get('fact_status') == False:
                        ...
                        await utils.answer(message, f'<emoji document_id=5325872701032635449>✅</emoji> <b>EvDC включен.</b>')
                        await self.evdcs()
                        return
                else:
    ...
    async def evdcstatistics(self, message):
    ...
    async def evdcgroup(self, message):
