...
class EVOlution(loader.Module):
    ...
    async def scfg(self, message):
        '''[ Параметр ] [ Значение ] - изменить параметр без открытия конфига\n> dc - изменить задержку для крафта\n> db - изменить задержку атаки босса\n> gi - изменить гиф в инлайн функциях\n> wt - время ожидания ответа от бота для evo\n> rc - чат, в который будут отправляться результаты\n> dcb - задержка между нажатиями на кнопку в sboss'''
    ...
    async def up(self, message):
        ...
        if args in uplist:
            ...
            await res.click(0)
            res = res.text
            await utils.answer(message, f'<emoji document_id=5204044038126182496>✅</emoji> Выполнено. Вот какие были условия:\n\n{res}')
            return
        else:
    ...
    async def sboss(self, message):
        ...
        if not reply_arg:
            ...
            return
        await utils.answer(message, f'<emoji document_id=5204044038126182496>✅</emoji> Выполняю! Выбран босс №{arg}')
