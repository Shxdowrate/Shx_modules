...
class EVOlution(loader.Module):
    ...
    async def up(self, message):
        ...
        args = utils.get_args_raw(message)
        if args in ['Ур','ур','Мщ','мщ','Уд','уд','Урон','урон','Крит','крит','Шп','шп','Уп','уп','Шк','шк','Мр','мр','Мп','мп','Мэ','мэ','Му','му','Зс','зс']:
            await utils.answer(message, '<b>[ upd ]</b> Выполняется...')
            async with self.client.conversation(self._backup_channel) as conv:
                await conv.send_message(f'{args}')
                res = await conv.get_response()
            await res.click(0)
            await utils.answer(message, '<b>[ upd ]</b> Выполнено. ')
            return
        else:
