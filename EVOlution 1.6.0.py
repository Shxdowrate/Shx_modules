...
class EVOlution(loader.Module):
    ...
    async def evo(self, message: Message):
        ...
        if args == '':
            ...
            await utils.answer(message, '<emoji document_id=5244840678340440877>🕗</emoji> Выполняется...')
    ...
    async def craft(self, message: Message):
        """ - скрафтить много бустеров"""
        await utils.answer(message, '<emoji document_id=5244840678340440877>🕗</emoji> Начинаю крафт бустеров...\n\nПодготовка данных... [ args, delay, type, mult, quantity, quantityl, errtype, errmult, startcraft ]')
            ...
            if mult in ['2', '2.0', '2.5', '3', '3.0']:
                while int(quantity) > 0:
                    ...
                    async with self.client.conversation(self._backup_channel) as conv:   
                        ...
                        await utils.answer(message, f'<emoji document_id=5244840678340440877>🕗</emoji> Начинаю крафт бустеров...\n\nПодготовка данных... [ args, delay, type, mult, quantity, quantityl, errtype, errmult, startcraft ] \n\nКрафтим...\n\nОтправлено запросов на данный момент: {startcraft}')
                await asyncio.sleep(2)    
                async with self.client.conversation(self._backup_channel) as conv:   
                        await conv.send_message(f'Бусты')
                        response = (await conv.get_response()).message
                        response = response[:-2]
                await utils.answer(message, '<emoji document_id=5244889456284019700>✅</emoji> Крафт завершен!\n\n\n<code>Загрузка информации...</code>')
                await asyncio.sleep(1)
                if type == 'д':
                    type = 'Деньги'
                elif type == 'р':
                    type = 'Руда'
                await utils.answer(message, f"<emoji document_id=5244889456284019700>✅</emoji> Крафт завершен!\n\n<b>Информация:</b>\nТип бустера: {type}\nМножитель бустера: {mult}\nКол-во раз отправлено: {quantityl}\nБыла задержка: {delay}\n\n<b>Итоговый список бустеров:</b>\n{response}")
            else:
    ...
    async def autoattack(self, message):
        ...
        if cfg == True:
            await utils.answer(message, '<emoji document_id=5244889456284019700>✅</emoji> АвтоАтакер включен.')
        if cfg == False:
    ...
    async def petroleum(self, message):
        ...
        if cfg == True:
            await utils.answer(message, '<emoji document_id=5244889456284019700>✅</emoji> Автодобыча нефти включена.')
            await self.client.send_message('@mine_evo_bot', 'Качать')
        if cfg == False:
    ...
    async def scfg(self, message):
        ...
        if param == 'db':
            ...
            await utils.answer(message, f'<emoji document_id=5244889456284019700>✅</emoji> Параметр <code>{param}</code> изменен на <code>{zz}</code>')
            return
        if param == 'dct':
    ...
    async def up(self, message):
        ...
        if args in uplist:
            ...
            await res.click(0)
            res = res.text
            await utils.answer(message, f'<emoji document_id=5244889456284019700>✅</emoji> Выполнено. Вот какие были условия:\n\n{res}')
            return
        else:
    ...
    async def sboss(self, message):
        '''[ Релпай на сообщение с выбором босса ] [ Число, под которым находится босс ] - автоматически выбрать босса при его появлении'''
        reply_arg = await message.get_reply_message()
        arg = utils.get_args_raw(message)
        delay_choose_boss = self.config['delay_choose_boss']
        if arg == '':
            await utils.answer(message, '<emoji document_id=5877477244938489129>🚫</emoji><b> Error | Ошибка</b>\nВы не выбрали босса!')
            return
        if not reply_arg:
            await utils.answer(message, '<emoji document_id=5877477244938489129>🚫</emoji><b> Error | Ошибка</b>\nВы не ответили на сообщение с выбором босса!')
            return
        await utils.answer(message, f'<emoji document_id=5244889456284019700>✅</emoji> Успешно! Выбран босс №{arg}')
