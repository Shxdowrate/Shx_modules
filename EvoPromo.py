...
class EvoPromo(loader.Module):
    ...
    async def promostatisticpromo(self, message):
        ...
        promo_groups = f'<emoji document_id=5936283232780684228>👥</emoji> | Группы для активации промо: <code>{utils.escape_html(self.get_prefix())}promogroups</code>\n'
        ...
        await utils.answer(message, text_on_top + promo_text + pref_text + command_activate + promo_groups + all_act + act + st_promo_text + qq + txt_senderss)
