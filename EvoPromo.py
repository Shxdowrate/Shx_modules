...
class EvoPromo(loader.Module):
                                ...
                                if args[3].isdigit():
                                    ...
                                    if args_3 > 0:
                                        ...
                                        if group != None:
                                            ...
                                            await utils.answer(message, f'<emoji document_id=5877477244938489129>🚫</emoji> <b>Error</b> | <code>{error_cmd}</code>\nВы не установили группу для промо. Используйте <code>{utils.escape_html(self.get_prefix())}psg</code> для установки группы.')
    ...
    @loader.command(alias = 'psg')
    async def promosetgroup(self, message):
        ''' - установить чат для промо'''
        error_cmd = f'<code>{utils.escape_html(self.get_prefix())}{inspect.currentframe().f_code.co_name}</code>'

        if message.is_private:
            await message.edit(f"<emoji document_id=5877477244938489129>🚫</emoji> <b>Error</b> | <code>{error_cmd}</code>\nЭто не группа.")
            return
        args = utils.get_args_raw(message)
        to_chat = None
    ...
    @loader.command(alias = 'stp')
    async def statisticpromo(self, message):
        ...
        await utils.answer(message, text_on_top + promo_text + pref_text + command_activate + all_act + act + st_promo_text + group + qq + txt_senderss)
